...
export function tenderObject(contract) {
  ...
  if (contract.TIPO_PROCEDIMIENTO) {
    Object.assign(document, {
      procurementMethod: tenderProcurementMethod(contract.TIPO_PROCEDIMIENTO, contract.PLANTILLA_EXPEDIENTE),
      procurementMethodDetails: contract.TIPO_PROCEDIMIENTO
    })
  }
...
function getStateName(initials) {
    switch(initials) {
        ...
        case 'COAH': return 'Coahuila de Zaragoza';
        ...
        case 'MICH': return 'Michoacán de Ocampo';
        ...
        case 'VER': return 'Veracruz de Ignacio de la Llave';
...
export function releaseObject({contract, metadata}) {
  // doc is a contract
  if (!contract.NUMERO_PROCEDIMIENTO || !contract.CODIGO_EXPEDIENTE) {
    return {};
  }
