...
export function tenderObject(contract) {
    ...
    const partyName = (contract.CONTRACT_UC_STRING && nombreDependencia != contract.CONTRACT_UC_STRING)? contract.CONTRACT_UC_STRING : nombreDependencia;
    const partyID = (contract.CONTRACT_UC_STRING && nombreDependencia != contract.CONTRACT_UC_STRING)? contract.CONTRACT_UC_STRING + ' ' + nombreDependencia : nombreDependencia;

    let tenderID = (contract.CONTRACT_CODE_STRING)? contract.CONTRACT_CODE_STRING.toString() : contract.CONTRACT_NUM_PROC_STRING.toString();
    const document = {
        id: tenderID,
        title: contract.CONTRACT_TITLE_STRING,
        status: 'complete',
        procuringEntity: {
            id: laundry.simpleName(laundry.launder(partyID)),
            name: partyName,
        }
    };

    if (contract.CONTRACT_TYPE_PROC_STRING) {
        let procurementMethod = tenderProcurementMethod(contract.CONTRACT_TYPE_PROC_STRING);
        if(procurementMethod != '') {
            Object.assign(document, { procurementMethod: procurementMethod });
        }
        Object.assign(document, { procurementMethodDetails: contract.CONTRACT_TYPE_PROC_STRING });
    }
...
function awardObject(contract) {
    let awardID = (contract.CONTRACT_CODE_STRING)? contract.CONTRACT_CODE_STRING.toString() : contract.CONTRACT_NUM_PROC_STRING.toString();
...
export function contractObject(contract) {
    let date = null;
    let contractID = (contract.CONTRACT_CODE_STRING)? contract.CONTRACT_CODE_STRING.toString() : contract.CONTRACT_NUM_PROC_STRING.toString();
...
export function supplierPartyObject(contract) {
    ...
    if(contract.CONTRACT_PROVIDER_ORG_STRING && (contract.ORG_COUNTRY_STRING || contract.ORG_STATE_STRING || contract.ORG_CITY_STRING || contract.ORG_ZONE_STRING || contract.ORG_STREET_STRING || contract.ORG_ZIPCODE_STRING)) {
        ...
        if(contract.ORG_STATE_STRING) address.region = fixStateName(contract.ORG_STATE_STRING);
    ...
    if(contract.CONTRACT_PROVIDER_ORG_STRING && (contract.ORG_PHONES_ARRAY || contract.ORG_WEBSITE_STRING || contract.ORG_EMAILS_ARRAY)) {
        ...
        if(contract.ORG_PHONES_ARRAY) contactPoint.telephone = contract.ORG_PHONES_ARRAY.toString();
...
function fixStateName(string) {
    switch(string) {
        case "D.F.": return "Ciudad de México";
