...
function valueObject(amount, currency, exchangeRate=null) {
    let obj = {
        amount: amount,
        currency: currency
    }
    if(exchangeRate) Object.assign( obj, { exchangeRate: exchangeRate } )

    return obj;
}

export function tenderObject(contract, convertedAmount) {
    ...
    if(convertedAmount !== null) {
        let originalValue = valueObject(contract.monto_estimado_sum, contract.moneda_monto_estimado, convertedAmount.exchangeRate);
        let convertedValue = valueObject(convertedAmount.convertedAmount, 'CRC');
        document.value = convertedValue;
        Object.assign(document, { originalValue: originalValue });
    }
...
function awardObject(contract, convertedAmount) {
    ...
    if(convertedAmount !== null) {
        let originalValue = valueObject(contract.monto, contract.moneda, convertedAmount.exchangeRate);
        let convertedValue = valueObject(convertedAmount.convertedAmount, 'CRC');
        awardObj.value = convertedValue;
        Object.assign(awardObj, { originalValue: originalValue });
    }
...
export function contractObject(contract, convertedAmount) {
    ...
    if(convertedAmount !== null) {
        let originalValue = valueObject(contract.monto, contract.moneda, convertedAmount.exchangeRate);
        let convertedValue = valueObject(convertedAmount.convertedAmount, 'CRC');
        contractObj.value = convertedValue;
        Object.assign(contractObj, { originalValue: originalValue });
    }
    ...
    if(contract.hasOwnProperty('proveedor') && contract.proveedor != null) {
        ...
        if(contract.proveedor.hasOwnProperty('direccion') && contract.proveedor.direccion != '') {
            ...
            if(country != "Costa Rica")
                ...
                Object.assign(party.address, { streetAddress: address[0], locality: address[1], region: address[2] });
        }
    }
    else {
...
function budgetObject(contract, convertedAmount) {
    if (contract.hasOwnProperty('monto_estimado')) {
        ...
        if(convertedAmount !== null) {
            let originalValue = valueObject(contract.monto_estimado_sum, contract.moneda_monto_estimado, convertedAmount.exchangeRate);
            let convertedValue = valueObject(convertedAmount.convertedAmount, 'CRC');
            obj.amount = convertedValue;
            Object.assign(obj, { originalAmount: originalValue });
        }
...
export function planningObject(contract, convertedAmount) {
    let planning = {
        budget: budgetObject(contract, convertedAmount),
    };
...
function convertAmount(amount, currency, date, exchangeRates) {
    let conversionDate = date.split('T')[0];
    let rate = 0;
    if(exchangeRates[currency + '-CRC'].hasOwnProperty(conversionDate)) {
        rate = parseFloat(exchangeRates[currency + '-CRC'][conversionDate]);
        return {
...
export function releaseObject({contract, metadata, exchangeRates}) {
    // doc is a contract
    ...
    // Convert amounts when exchange rates apply
    let convertedContractAmount = null;
    if(contract.moneda != 'CRC') {
        convertedContractAmount = convertAmount(contract.monto, contract.moneda, contract.fecha_notificacion, exchangeRates);
    }
    let convertedBudgetAmount = null;
    if(contract.moneda_monto_estimado != 'CRC' && contract.monto_estimado_sum > 1) {
        convertedBudgetAmount = convertAmount(contract.monto_estimado_sum, contract.moneda_monto_estimado, contract.fecha_notificacion, exchangeRates);
    }
    ...
    const release = {
        ...
        tender: tenderObject(contract, convertedBudgetAmount),
        awards: [
            awardObject(contract, convertedContractAmount),
        ],
        contracts: [
            contractObject(contract, convertedContractAmount),
        ],
        publisher: {
    ...
    const planningObj = planningObject(contract, convertedBudgetAmount);
    if(planningObj) {
